...
function pixisCheckStock(productName, currentQty) {
  const stockMax = pixisGetStockDisponible(productName);
  ...
  cart.forEach((item, index) => {
    ...
    cartItems.innerHTML += `
      <div class="cart-item">
        ...
        <button class="cart-item-remove" onclick="removeCartItem(${index})" title="Eliminar producto">&times;</button>
      </div>`;
  });
...
btnFinish?.addEventListener('click', e => {
  ...
  if (envioDomicilio.checked && (!nombre || !localidad || !direccion || !telefono)) {
    alert('Para envíos debés completar los datos (incluyendo celular).');
    return;
  }
  ...
  if (envioDomicilio.checked) {
    msg += `%0A👤 ${nombre}%0A📞 Celular: ${telefono}%0A📍 ${direccion}%0A%0A🏙️ ${localidad}%0A🗺️ Santiago del Estero%0A`;
  } else {
    if (nombre || telefono) {
...
window.openProductModal = function (card, pushToHistory = true) {
  ...
  if (pushToHistory) {
    const prodId = card.dataset.pixisSlug || card.dataset.pixisId || card.dataset.title.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
...
function closeModal(fromHistory = false) {
  ...
  // [FIX] Guardar posición de scroll y búsqueda activa ANTES de tocar el historial,
  // porque pushState/back pueden disparar syncAppStateFromUrl → goHome() → scrollTo(0)
  const savedScrollY = window.scrollY;
  const searchInp = document.getElementById('searchInput');
  const savedSearch = searchInp ? searchInp.value : '';
  ...
  // Si fromHistory=true, el sistema (popstate→syncAppState→goHome) ya maneja la navegación.
  if (fromHistory !== true) {
    requestAnimationFrame(() => {
      window.scrollTo({ top: savedScrollY, left: 0, behavior: 'instant' });
      // Restaurar búsqueda si se limpió por el popstate → goHome()
...
function generarPDFPresupuesto() {
  ...
  // CELULAR
  doc.setFont("helvetica", "bold"); doc.text("CELULAR:", 10, yCliente + 15);
  doc.setFont("helvetica", "normal"); doc.text(clienteTelefono, 33, yCliente + 15);

  // LOCALIDAD (reemplaza teléfono)
  doc.setFont("helvetica", "bold"); doc.text("LOCALIDAD:", 10, yCliente + 20);
  doc.setFont("helvetica", "normal"); doc.text(localidad, 33, yCliente + 20);

  // ENVIO A DOMICILIO (reemplaza email)
  doc.setFont("helvetica", "bold"); doc.text("ENVIO A DOMICILIO:", 10, yCliente + 25);
  doc.setFont("helvetica", "normal");
  doc.text(tieneEnvio ? "Si" : "No - RETIRO EN LOCAL", 50, yCliente + 25);
